...
hooks:          []
auto_task:      "1. go vet ./... && go test ./... && go build ./cmd/dalcenter/ && go build ./cmd/dalcli/ 실행. 2. dalcli ps → 30분+ idle 멤버 감지 시 leader에게 보고. 3. gh pr list --state open → 24시간+ 방치 PR 감지 시 leader에게 보고. 실패 항목은 정리해서 보고, 전부 통과하면 PASS."
auto_interval:  "30m"
git: {
    user:         "dal-${name}"
