...
# 3. Docker 컨테이너
pct exec 105 -- docker ps --format "table {{.Names}}\t{{.Status}}"
...
```bash
pct exec 105 -- docker stats --no-stream --format "table {{.Name}}\t{{.CPUPerc}}\t{{.MemUsage}}"
```
...
exp = d['claudeAiOauth']['expiresAt']
remaining = (exp/1000) - datetime.datetime.now().timestamp()
print(f'LXC 105: {int(remaining/3600)}h {int(remaining%3600/60)}m 남음')
"
