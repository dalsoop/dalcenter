...
pct exec 105 -- dalcenter tell <repo> "dev 깨워"
pct exec 105 -- dalcenter tell <repo> "전부 재워"
```
...
pct exec 105 -- curl -s http://localhost:<port>/api/status

# 특정 dal 로그
pct exec 105 -- docker logs --since 30m <container-name> 2>&1 | tail -20

# 컨테이너 진입
pct exec 105 -- docker exec -it <container-name> bash
```
...
```bash
# 현재 이미지 목록
pct exec 105 -- docker images | grep dalcenter

# 이미지 빌드
pct exec 105 -- dalcenter image build
...
```bash
pct exec 105 -- cat /etc/dalcenter/<repo>.env
```
