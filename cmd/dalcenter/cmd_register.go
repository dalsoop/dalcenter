...
func newRegisterCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&dalbridgeURL, "dalbridge-url", os.Getenv("DALCENTER_DALBRIDGE_URL"), "Dalbridge URL for dal containers (webhook stream)")
...
func installSystemdService(serviceName, repoName, repoPath string, port int, bridgeURL, dalbridgeURL string) error {
	...
	if dalbridgeURL != "" {
		envContent += fmt.Sprintf("DALCENTER_DALBRIDGE_URL=%s\n", dalbridgeURL)
	}
