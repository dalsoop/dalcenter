...
func TestIssueWorkflowStore_Eviction(t *testing.T) {
	...
	// Fill up to limit with completed workflows
...
func TestHandleIssueWorkflow_AcceptsValid(t *testing.T) {
	...
	// Async workflow will fail (no Docker, no dal.cue), but the request should succeed
	...
	// Should be failed since there's no dal.cue and no Docker
	if wf.Status != "failed" {
...
func TestCaptureResponseWriter(t *testing.T) {
	...
	// Second call should return same header
	h2 := w.Header()
	h.Set("X-Test", "val")
...
func TestIssueWorkflow_FullFlow_WakeFailure(t *testing.T) {
	// Workflow should fail gracefully when wake fails (no dal.cue)
	d := &Daemon{
		issueWorkflows: newIssueWorkflowStore(),
		localdalRoot:   t.TempDir(), // empty dir = no dal.cue
		containers:     map[string]*Container{},
		tasks:          newTaskStore(),
		feedback:       newFeedbackStore(),
		costs:          newCostStore(),
	...
	// Should have events tracking the phases
	hasWakeEvent := false
	hasFailEvent := false
...
func TestIssueWorkflow_FullFlow_WithBridgeNotification(t *testing.T) {
	...
	// Wake fails (no dal.cue), but bridge notification should still be sent
	if wf.Status != "failed" {
		t.Fatalf("expected status=failed, got %s", wf.Status)
	}
...
func TestIssueWorkflow_DuplicateAllowedAfterCompletion(t *testing.T) {
	...
	// Should allow creating another for the same issue
	wf2 := s.Create("529", "dev", "task2")
	if wf2.ID == wf1.ID {
		t.Fatal("expected different workflow IDs")
	}

	// FindByIssue should return the new active one
	found := s.FindByIssue("529")
	if found == nil {
...
func TestIssueWorkflow_HTTPEndToEnd(t *testing.T) {
	...
	// Wait for async workflow to complete (will fail due to no Docker)
	...
	// Step 4: Starting same issue again should now work (previous one failed)
...
func TestHandleIssueWorkflow_RequiresAuth(t *testing.T) {
	...
	// Will be 400 (missing fields validation) or 202, but NOT 401
