...
type IssueWorkflow struct {
	...
	Member    string    `json:"member"`              // dal name to wake for this issue
	Task      string    `json:"task"`                // task description for the member
	Status    string    `json:"status"`              // "pending", "waking", "assigned", "working", "done", "failed"
	Phase     string    `json:"phase"`               // current phase description
	TaskID    string    `json:"task_id,omitempty"`    // tracked task ID (from /api/task)
	PRUrl     string    `json:"pr_url,omitempty"`     // PR URL if created
...
func (d *Daemon) handleIssueWorkflow(w http.ResponseWriter, r *http.Request) {
	var req struct {
		IssueID      string `json:"issue_id"`
		Member       string `json:"member"`        // which dal to wake (e.g. "dev")
		Task         string `json:"task"`           // task instruction
		CallbackPane string `json:"callback_pane"`  // for dalroot notification
	}
